import { ADD_EXPENSES, SET_INITIAL_TRANSACTION_STATE, REMOVE_TRANSACTION_STATE, GET_ALL_TRANSACTION, CURRENT_USER } from '../../utils/constant';

const initialState = {
  transactions:
    localStorage.getItem(GET_ALL_TRANSACTION) && localStorage.getItem(CURRENT_USER) ? JSON.parse(localStorage.getItem(GET_ALL_TRANSACTION)) : [],
};
...
const transactionReducer = (newstate = initialState, action) => {
  ...
  if (action.type === ADD_EXPENSES) {
    ...
    if (totalPercentage != 100) {
      ...
      const allTransactions = JSON.parse(localStorage.getItem('all-transaction')) || [];
