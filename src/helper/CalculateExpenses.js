...
export const CalculateExpenses = () => {
  ...
  personlisedTransaction.forEach((transact) => {
    ...
    if (transact.paidBy === currentUser.username) {
      transact.students.map((transactPaidFor) => (userExpenses[transactPaidFor.paidFor] += transact.amount * (transactPaidFor.percentage / 100)));
    } else {
      ...
      transact.students.map((transactPaidFor) => {
        if (transactPaidFor.paidFor === currentUser.username) {
          userExpenses[transact.paidBy] -= transact.amount * (transactPaidFor.percentage / 100);
          userExpenses[transactPaidFor.paidFor] += transact.amount * (transactPaidFor.percentage / 100);
        }
      });
    }
  });
